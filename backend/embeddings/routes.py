...
def generate_embedding():
    ...
    try:
        ...
        collection.add(
            ids=[str(uuid.uuid4())], documents=[text], embeddings=[embedding]
        )

        return jsonify(
            {"message": "Embedding stored successfully!", "embedding": embedding}
        )
...
def delete_embedding():
    ...
    try:
        ...
        return (
            jsonify({"message": f"Embedding {embedding_id} deleted successfully!"}),
            200,
        )
